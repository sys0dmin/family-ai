...
async def process_image_turn(
    ...
    request_trace_registry.start(request_id, "vision")
    content_type = (file.content_type or "").split(";", maxsplit=1)[0].lower()
    if content_type not in ALLOWED_IMAGE_TYPES:
        request_trace_registry.finish(request_id, "error", error_code="http_415")
        raise HTTPException(
    ...
    if len(content) > service.max_image_bytes:
        ...
        request_trace_registry.finish(request_id, "error", error_code="http_413")
        raise HTTPException(
    ...
    if not normalized_question:
        request_trace_registry.finish(request_id, "error", error_code="http_422")
        raise HTTPException(
    ...
    try:
        ...
        request_trace_registry.finish(request_id, "error", error_code="http_403")
        raise HTTPException(
        ...
        request_trace_registry.finish(request_id, "error", error_code="http_503")
        raise HTTPException(
