...
- версию и SHA-256 валидированного каталога игровой больницы.
...
### Каталог игровой больницы

Gateway публикует `schema_version` и SHA-256 канонического содержимого
валидированного clinic catalog. Admin вычисляет ожидаемое значение из своего
release независимо. Drift означает, что процессы Gateway и Admin собраны с
разными игровыми сценариями; исходный текст сценариев в паспорт не попадает.

## Проверка после release

1. Открыть «Инфраструктура» и нажать «Обновить».
2. Gateway, Speech, PostgreSQL schema, конфигурация и Clinic catalog должны быть
   зелёными.
