...
На DB и Speech `node_exporter` слушает `0.0.0.0:9100`, а доступ к порту
ограничивает `nftables` только loopback и Gateway. Привязка exporter к адресу
конкретного LXC намеренно не используется: при раннем старте systemd этот адрес
может ещё отсутствовать, из-за чего мониторинг не переживёт перезагрузку.
