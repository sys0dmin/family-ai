...
`pip-audit 2.10.1` и обращается только к публичной базе advisory. При временном
сетевом сбое он делает до трёх попыток; отчёт о найденной уязвимости не
повторяется и немедленно блокирует релиз. Исходники,
