...
Агент «Нотка» умеет сочинять с ребёнком новые песни и получает опциональный инструмент `music_recognition`. Только его голосовые ходы могут отправляться в ACRCloud. Исходное аудио не сохраняется; если инструмент выключен или не нашёл совпадение, агент не выдумывает ответ, а просит напеть ещё раз или добавить слова.
